  ...
  _data.list('checks',function(err,checks){
    if(!err && checks && checks.length > 0){
      checks.forEach(function(check){
        ...
        _data.readFile('checks',check,function(err,originalCheckData){
          if(!err && originalCheckData){
            // Pass it to the check validator, and let that function continue the function or log the error(s) as needed
            workers.validateCheckData(JSON.parse(originalCheckData));
          } else {
...
workers.validateCheckData = function(originalCheckData){
  ...
  originalCheckData.successCodes = originalCheckData.successCodes ? originalCheckData.successCodes : [200,201];
  originalCheckData.timeoutSeconds = typeof(originalCheckData.timeoutSeconds) == 'number' && originalCheckData.timeoutSeconds % 1 === 0 && originalCheckData.timeoutSeconds >= 1 && originalCheckData.timeoutSeconds <= 7 ? originalCheckData.timeoutSeconds : false;
...
workers.processCheckOutcome = function(originalCheckData,checkOutcome){
  ...
  // Save the updates
  _data.updateFile('checks',newCheckData.id,newCheckData,function(err){
    if(!err){
      // Send the new check data to the next phase in the process if needed
      if(alertWarranted){
  		workers.log(originalCheckData,checkOutcome,state,alertWarranted)
...
workers.log = (originalCheckData,checkOutcome,state,alertWarranted)=>{
	...
	var stringData = JSON.stringify(logData);
	var logFileName = originalCheckData.id;

	_logs.append(logFileName,stringData,(err)=>{
		if(err){
			debug('error comes in logging the file')
		}else{
	...
	_logs.list(false,(err,logs)=>{
		logs.forEach((log)=>{
			...
			_logs.compress(logId,newFileId,(err)=>{
				if(err){
					debug('error comes in compresing the one of the log file')
				}else{
					_logs.truncate(logId,(err)=>{
						if(err){
							debug('error comes in truncating on of the logging file')
						}else{
