						...
						if(userChecks.length < 5){
							...
							myFileLib.createFile('checks',checkId,checkObject,(err)=>{
								if(err){
									...
									var newUserData = Object.assign({},userData,{checks:userChecks});
...
handlers._checks.get = (data,callback)=>{
  ...
  query = data.query.checkId
  if(query){
    if(token){
      myFileLib.readFile('tokens',token,(err,tokenData)=>{
        if(err){
          callback(403,{message:'cannot found the token file'})
        }else{
          tokenData = JSON.parse(tokenData)
          handlers._tokens.verify(token,tokenData.phone,(isValid)=>{
        if(isValid){
          myFileLib.readFile('checks',query,(err,checkData)=>{
            if(err){
              callback(403,{message:err})
            }else{
              checkData = typeof checkData === 'object' ? checkData :  JSON.parse(checkData)
      ...
      callback(403,{message:"cannot found the token in the headers"})
    }
  }else{
    callback(403,{message:'cannot found the check id in query'})
  }
}
...
handlers._checks.put = function(data,callback){
  ...
  var checkId = data.query.checkId 
  ...
  if(checkId){
    ...
    if(protocol || url || method || successCodes || timeoutSeconds){
      // Lookup the check
     myFileLib.readFile('checks',checkId,function(err,checkData){
        if(!err && checkData){
          ...
          handlers._tokens.verify(token,checkData.userPhone,function(tokenIsValid){
            if(tokenIsValid){
              ...
              myFileLib.updateFile('checks',checkId,checkData,function(err){
                if(!err){
                  callback(200);
                } else {
      ...
      if(!err && checkData){
        ...
        handlers._tokens.verify(token,checkData.userPhone,function(tokenIsValid){
          if(tokenIsValid){
            ...
            myFileLib.deleteFile('checks',checkId,function(err){
              if(!err){
                // Lookup the user's object to get all their checks
                myFileLib.readFile('users',checkData.userPhone,function(err,userData){
                  if(!err){
                  userData = typeof userData === 'object' ? userData : JSON.parse(userData)
                    ...
                    if(checkPosition > -1){
                      ...
                      myFileLib.updateFile('users',checkData.userPhone,userData,function(err){
                        if(!err){
